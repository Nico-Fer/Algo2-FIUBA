...
nodo_t* buscar_nodo_lista(nodo_t* nodo_actual, nodo_t* nodo_a_buscar){
	if((*nodo_actual).siguiente != nodo_a_buscar){
		return buscar_nodo_lista((*nodo_actual).siguiente, nodo_a_buscar);
	}	
	return nodo_actual;
}	
...
void borrar_unico_elemento_en_lista(lista_t* lista, lista_liberar_elemento destructor){
	if(lista->destructor)
...
int lista_borrar(lista_t* lista, lista_liberar_elemento destructor){
	if(!lista || (*lista).cantidad==CANTIDAD_NULA)
		return ERROR;
	if((*lista).cantidad==UN_ELEMENTO){
		borrar_unico_elemento_en_lista(lista, destructor);
		return EXITO;
	}	
	nodo_t* nodo_a_designar = buscar_nodo_lista((*lista).nodo_inicio, (*lista).nodo_fin);
	if(lista->destructor)
		destructor(lista->nodo_fin->elemento);
	free((*lista).nodo_fin);
...
void lista_borrar_principio(lista_t* lista, lista_liberar_elemento destructor){
	if((*lista).cantidad==UN_ELEMENTO){
		borrar_unico_elemento_en_lista(lista, destructor);
		return ;
	}
...
void borrar_en_posicion(lista_t* lista, size_t posicion, lista_liberar_elemento destructor){
...
int lista_borrar_de_posicion(lista_t* lista, size_t posicion, lista_liberar_elemento destructor){
	if(!lista || (*lista).cantidad==CANTIDAD_NULA)
		return ERROR;
	if(posicion<CANTIDAD_NULA || posicion>=(*lista).cantidad){
		lista_borrar(lista, destructor);
	}else if(posicion==POSICION_PRINCIPIO){
		lista_borrar_principio(lista, destructor);
	}else{	
		borrar_en_posicion(lista, posicion, destructor);
	}
	return EXITO;	
}
...
int lista_desapilar(lista_t* lista){
	if(!lista)
		return ERROR;
	lista_borrar_principio(lista, lista->destructor);
	return EXITO;
}	
